...
class PurchaseManagementSystem:
    ...
    def create_purchase_entry(self, user_id: int, party_id: str, items: List[Dict], 
                            ...
                            notes: str = '', order_no: str = None, bill_no: int = None,
                            bill_date: str = None) -> Dict:
        """Create a complete purchase entry with multiple items"""
        try:
            # Use provided bill number/date when available
            bill_number = bill_no or self._generate_bill_number(user_id)
            bill_dt = date.today()
            if bill_date:
                try:
                    bill_dt = datetime.strptime(bill_date, '%Y-%m-%d').date()
                except Exception:
                    bill_dt = date.today()
            ...
            for item_data in items:
                ...
                katta = float(item_data.get('katta') or 0)
                rate = float(item_data.get('rate') or 0)
                discount_percent = float(item_data.get('discount_percent') or item_data.get('discount') or 0)
                tax_percent = float(item_data.get('tax_percent') or item_data.get('taxp') or 0)

                if not item_code:
                    raise ValueError('Item code missing in payload')
                ...
                total_weight = quantity * (katta if katta else 1)
                base_amount = total_weight * rate
                discount_val = base_amount * (discount_percent / 100)
                tax_val = base_amount * (tax_percent / 100)
                net_amount = base_amount + tax_val - discount_val
            ...
            # Optional transport/other charges as separate line
            ...
            return {
                'success': True,
                'message': f'Purchase entry created successfully with bill number {bill_number}',
                'bill_no': bill_number,
    ...
    def get_purchase_list(
        ...
        try:
            ...
            if end_date:
                query = query.filter(Purchase.bill_date <= datetime.strptime(end_date, '%Y-%m-%d').date())
            if party_id:
                query = query.filter(Purchase.party_cd == party_id)
            if item_id:
                query = query.filter(Purchase.it_cd == item_id)
            ...
            for bill_no in pagination.items:
                ...
                if bill_purchases:
                    ...
                    first_item_cd = bill_purchases[0].it_cd
                    item_name = None
                    try:
                        item_obj = Item.query.filter_by(it_cd=first_item_cd, user_id=user_id).first()
                        item_name = item_obj.it_nm if item_obj else None
                    except Exception:
                        item_name = None
                    ...
                    'party_name': party.party_nm if party else '',
                    'item_code': first_item_cd,
                    'item_name': item_name,
                    'qty': float(bill_purchases[0].qty or 0),
                    'weight': float((bill_purchases[0].tot_smt or 0) or (bill_purchases[0].qty or 0) * (bill_purchases[0].katta or 0)),
                    'rate': float(bill_purchases[0].rate or 0),
                    'sal_amt': float(bill_purchases[0].sal_amt or 0),
                    'total_amount': float(total_amount),
