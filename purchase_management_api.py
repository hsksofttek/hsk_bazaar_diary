...
def create_purchase_entry():
    ...
    try:
        ...
        party_id = data.get('party_id') or data.get('party_cd')

        items_payload = data.get('items')
        if not items_payload:
            items_payload = [{
                'item_code': data.get('it_cd'),
                'quantity': data.get('qty', 0),
                'katta': data.get('katta', 0),
                'rate': data.get('rate', 0),
                'discount_percent': data.get('cdisp', 0),
                'tax_percent': data.get('taxp', 0)
            }]

        extra_charges = (float(data.get('adat') or 0) +
                         float(data.get('hamali') or 0) +
                         float(data.get('other') or 0))
        ...
        result = pms.create_purchase_entry(
            ...
            transport_charges=extra_charges,
            payment_terms=data.get('payment_terms', ''),
            delivery_date=data.get('delivery_date') or data.get('order_dt'),
...
def get_purchase_statistics():
    ...
    try:
        result = pms.get_purchase_statistics(current_user.id)
        return jsonify(result), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400


@purchase_management_api.route('/api/purchase/next-bill', methods=['GET'])
@login_required
def get_next_bill_number():
    """Get next available bill number for the current user"""
    try:
        bill_no = pms._generate_bill_number(current_user.id)
        return jsonify({'success': True, 'bill_no': bill_no}), 200
    except Exception as e:
