...
@login_required
def get_parties():
    """Get all parties with pagination and search (user-specific)"""
...
def get_party(party_cd):
    """Get specific party by code (user-specific)"""
    party = Party.query.filter_by(party_cd=party_cd, user_id=current_user.id).first_or_404()
    return jsonify(serialize_party(party))

@api_bp.route('/parties', methods=['POST'])
@login_required
def create_party():
    """Create new party (user-specific)"""
    data = request.get_json()
    
    if not data:
        return jsonify({'success': False, 'message': 'No data provided'}), 400
    
    # Check if party code already exists for this user
    if Party.query.filter_by(party_cd=data.get('party_cd'), user_id=current_user.id).first():
...
@login_required
def get_items():
    """Get all items with pagination and search (user-specific)"""
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 20, type=int)
    search = request.args.get('search', '')
...
@login_required
def get_item(it_cd):
    """Get specific item by code (user-specific)"""
    item = Item.query.filter_by(it_cd=it_cd, user_id=current_user.id).first_or_404()
...
def create_item():
    ...
    # Check if item code already exists for this user
    if Item.query.filter_by(it_cd=data.get('it_cd'), user_id=current_user.id).first():
...
@login_required
def get_purchases():
    """Get all purchases with pagination (user-specific)"""
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 20, type=int)
...
def get_purchase(purchase_id):
    """Get specific purchase by ID (user-specific)"""
    purchase = Purchase.query.filter_by(id=purchase_id, user_id=current_user.id).first_or_404()
...
def create_single_item_purchase(data):
    """Create single item purchase (legacy support) - user-specific"""
    try:
        purchase = Purchase(
            user_id=current_user.id,
...
def create_multi_item_purchase(data):
    """Create multi-item purchase - user-specific"""
    try:
        bill_no = data.get('bill_no')
        bill_date = datetime.strptime(data.get('bill_date'), '%Y-%m-%d').date() if data.get('bill_date') else date.today()
        party_cd = data.get('party_cd')
        ...
        existing_purchase = Purchase.query.filter_by(bill_no=bill_no, user_id=current_user.id).first()
...
@login_required
def get_sales():
    """Get all sales with pagination (user-specific)"""
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 20, type=int)
...
def get_sale(sale_id):
    """Get specific sale by ID (user-specific)"""
...
def create_single_item_sale(data):
    """Create single item sale (legacy support) - user-specific"""
    try:
        sale = Sale(
            user_id=current_user.id,
            bill_no=data.get('bill_no'),
...
def create_multi_item_sale(data):
    """Create multi-item sale - user-specific"""
    try:
        bill_no = data.get('bill_no')
        bill_date = datetime.strptime(data.get('bill_date'), '%Y-%m-%d').date() if data.get('bill_date') else date.today()
        party_cd = data.get('party_cd')
        ...
        # Check if bill number already exists for this user
        existing_sale = Sale.query.filter_by(bill_no=bill_no, user_id=current_user.id).first()
        if existing_sale:
            return jsonify({'success': False, 'message': 'Bill number already exists'}), 400
