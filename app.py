...
def create_app():
    ...
    @app.route('/sales-management-legacy')
    @login_required
    def sales_management_legacy():
        """Legacy Advanced Sales Management page"""
        return render_template('sales_management.html')

    @app.route('/sales-management')
    @login_required
    def sales_management():
        """Modern Sales Management page"""
        return render_template('sales_enhanced.html')
    
    @app.route('/inventory-management')
    @login_required
    def inventory_management():
        """Advanced Inventory Management page with sidebar wrapper"""
        return render_template('inventory_management_wrapper.html')
    
    @app.route('/inventory-management-legacy')
    @login_required
    def inventory_management_legacy():
        """Legacy Inventory Management page"""
        return render_template('inventory_management.html')
    
    @app.route('/financial-management')
    @login_required
    def financial_management():
        """Advanced Financial Management page with sidebar wrapper"""
        return render_template('financial_management_wrapper.html')
    
    @app.route('/financial-management-legacy')
    @login_required
    def financial_management_legacy():
        """Legacy Financial Management page"""
        ...
        try:
            ...
            return jsonify({
                ...
                'parties': [{
                    ...
                    'address': " ".join(filter(None, [party.address1 or '', party.address2 or '', party.place or ''])).strip(),
                    'phone': party.phone or party.phone1 or party.mobile,
                    'email': party.email,
                    'balance': float(party.current_balance or party.closing_bal or 0)
    ...
    def print_bill(bill_type, bill_no):
        ...
        try:
            if bill_type == 'purchase':
                ...
                # Get party details
                party = Party.query.filter_by(party_cd=bill.party_cd, user_id=current_user.id).first()
...
if __name__ == '__main__':
    ...
    host = '0.0.0.0' if os.environ.get('PORT') else os.environ.get('FLASK_RUN_HOST', '127.0.0.1')
