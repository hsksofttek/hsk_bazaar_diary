...
def get_sales_list():
    ...
    try:
        ...
        sales = []
        for entry in result.get('sales', []):
            sales.append({
                'bill_no': entry.get('bill_no'),
                'bill_date': entry.get('bill_date'),
                'party_cd': entry.get('party_code'),
        ...
        total_count = pagination.get('total', len(sales))
